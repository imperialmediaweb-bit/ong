  ...
  if (!ngoId) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <Card className="max-w-md w-full">
          <CardHeader className="text-center">
            <CardTitle>{isSuperAdmin ? "Nicio organizatie selectata" : "Nicio organizatie asociata"}</CardTitle>
            <CardDescription>
              {isSuperAdmin
                ? "Contul tau de Super Admin nu este asociat cu o organizatie. Foloseste panoul de administrare pentru a gestiona platforma."
                : "Contul tau nu este asociat cu nicio organizatie. Contacteaza administratorul platformei."}
            </CardDescription>
          </CardHeader>
          {isSuperAdmin && (
            <CardContent className="flex justify-center">
              <Link href="/admin">
                <Button>Mergi la Panoul Admin</Button>
              </Link>
            </CardContent>
          )}
        </Card>
