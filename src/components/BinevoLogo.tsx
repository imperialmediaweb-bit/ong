...
export function BinevoLogo({
  ...
  return (
    <span className={`inline-flex items-center ${s.gap} ${className}`}>
      ...
      >
        <defs>
          <linearGradient id="binevo-heart-gradient" x1="0" y1="0" x2="64" y2="64" gradientUnits="userSpaceOnUse">
        ...
        <path
          ...
          fill="none"
        />
        {/* Inner smile/check accent */}
        <path
          d="M22 30C22 30 26 36 32 36C38 36 42 30 42 30"
          stroke="url(#binevo-heart-gradient)"
          strokeWidth="3"
          strokeLinecap="round"
          fill="none"
      ...
      {showText && (
        <span
          className={`${s.text} font-bold ${
            textColor || "bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent"
          }`}
        >
          Binevo
        </span>
  ...
  return `
    <div style="display:inline-flex;align-items:center;gap:8px;">
      <svg width="${width * 0.4}" height="${width * 0.4}" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <path d="M22 30C22 30 26 36 32 36C38 36 42 30 42 30" stroke="url(#hg)" stroke-width="3" stroke-linecap="round" fill="none"/>
      </svg>
      <span style="font-size:${width * 0.22}px;font-weight:700;background:linear-gradient(135deg,#6366F1,#A855F7);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">Binevo</span>
    </div>
  `;
}
